...
EmergentStop emergency_stop;


static void CallbackEmergentStopState(CanStdDataFrame_t &cmd) {
  emergency_stop.SetStatus(cmd.data[0]);
}


ErrCode EmergentStop::Init(MAC_t &mac, uint8_t mac_index) {
  ErrCode ret;
  ...
  for (int i = 0; i < cmd.data[MODULE_EXT_CMD_INDEX_DATA]; i++) {
    ...
    if (function.id == FUNC_REPORT_EMERGENCY_STOP)
      message_id[i] = canhost.RegisterFunction(function, CallbackEmergentStopState);
    else
...
void EmergentStop::PollState() {
  CanStdFuncCmd_t cmd;

  cmd.id = FUNC_REPORT_EMERGENCY_STOP;
  cmd.length = 0;

  canhost.SendStdCmd(cmd, 0);
}

void EmergentStop::Process() {
  if (!IsOnline())
    return;
  ...
  if (event_state_ != EMERGENCY_STOP_TRIGGER) {
    if (state_ == EMERGENCY_STOP_TRIGGER) {
      ...
      ModuleBase::LockMarlinUart(LOCK_SOURCE_EMERGENT_STOP);
      ReportStatus();
...
ErrCode EmergentStop::ReportStatus() {
